...
func InitSql() {
	if runMode, err := config.String("runmode"); err != nil {
		logs.Error("beego.AppConfig.String runmode err:", err)
		return
	} else if runMode == "dev" {
		orm.Debug = true
	}
	...
	dbDriver, err := config.String("dbdriver")
	if err != nil {
		logs.Error("beego.AppConfig.String dbdriver err:", err)
		return
	}
	if dbDriver == "mysql" {
		...
		passwd, _ := config.String("mysqlpass")
		host, _ := config.String("mysqlurls")
	...
	if err := orm.RegisterDataBase("default", dbDriver, dataSource, orm.MaxIdleConnections(maxIdle), orm.MaxOpenConnections(maxConn)); err != nil {
...
func InitDbFrameData() {
	// 自动建表
	autoCreateDb := config.DefaultInt("dbautocreate", 0)
