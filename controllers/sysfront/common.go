...
func (c *CommonFrontController) Upload() {
	...
	defer f.Close()
	if err != nil {
		logs.Error("Recharge upload file get file error1", err)
		c.Msg = "上传失败，请重试(1)"
		return
	}
	...
	if flag, _ := utils.PathExists(uploadPath); !flag {
		if err2 := os.MkdirAll(uploadPath, 0644); err2 != nil {
			logs.Error("Recharge upload file get file error2", err2)
			c.Msg = "上传失败，请重试(2)"
			return
		}
	}
	...
	if err3 != nil {
		logs.Error("Recharge upload file save file error3", err3)
		c.Msg = "上传失败，请重试(3)"
		return
	}
	c.Code = 0
