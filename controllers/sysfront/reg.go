...
func (c *RegFrontController) Post() {
	...
	// 验证用户名是否存在
	om := orm.NewOrm()
	if isExist := om.QueryTable(new(Member)).Filter("Username", model.Username).Exist(); isExist {
		c.Msg = "手机号已存在"
		return
	}
	// 查询层级
	...
	if refId != 0 {
		var refMem Member
		if err := om.QueryTable(new(Member)).Filter("Id", refId).One(&refMem, "Levels", "LevelsDeep"); err != nil {
	...
	om.DoTx(func(ctx context.Context, txOrm orm.TxOrmer) error {
		...
		var memberId int64
		var err error
		if memberId, err = txOrm.Insert(&model); err != nil {
