...
func (c *SiteConfigIndexController) Delone() {
	...
	if err1 != nil {
		logs.Error("Delete siteconfig error", err1)
		msg = "删除失败"
	} else {
		code = 1
		msg = "删除成功"
...
func (c *SiteConfigAddController) Post() {
	...
	if created, _, err := o.ReadOrCreate(&siteConfig, "Code"); err != nil {
		msg = "添加失败,请重试"
		logs.Error("Insert siteconfig error", err)
	} else if !created {
		msg = "添加失败，配置已存在"
	} else {
		code = 1
...
func (c *SiteConfigEditController) Get() {
	...
	if err == orm.ErrNoRows || err == orm.ErrMissPK {
		c.Redirect(web.URLFor("SiteConfigIndexController.get"), 302)
	} else {
		c.Data["codeMap"] = map[string]string{
			"DIY":        "自定义",
			utils.Scname: "站点名称",
...
func (c *SiteConfigEditController) Post() {
	...
	if _, err := o.Update(&siteConfig, "Value", "ModifyDate"); err != nil {
		msg = "更新失败"
		logs.Error("Update siteconfig error", err)
	} else {
		code = 1
		msg = "更新成功"
	}
