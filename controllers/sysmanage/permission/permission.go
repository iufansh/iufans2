...
func (c *PermissionIndexController) Delone() {
	...
	if err1 != nil {
		logs.Error("Delete permission error", err1)
		msg = "删除失败"
	} else {
		code = 1
		msg = "删除成功"
...
func (c *PermissionAddController) Post() {
	...
	if _, err := o.Insert(&permission); err != nil {
		msg = "添加失败"
		logs.Error("Insert permission error", err)
	} else {
		code = 1
		msg = "添加成功"
	}
...
func (c *PermissionEditController) Get() {
	...
	if err == orm.ErrNoRows || err == orm.ErrMissPK {
		c.Redirect(web.URLFor("PermissionIndexController.get"), 302)
	} else {
...
func (c *PermissionEditController) Post() {
	...
	if num, err := o.Update(&permission, cols...); err != nil {
		msg = "更新失败"
		logs.Error("Update permission error", err)
	} else if num == 0 {
		msg = "更新失败"
	} else {
		code = 1
