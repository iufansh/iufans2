...
func (c *OrganizationIndexController) Delone() {
	var code int
	var msg string
	url := web.URLFor("OrganizationIndexController.Get")
	...
	if err1 != nil {
		logs.Error("Delete Organization error", err1)
		msg = "删除失败"
	} else {
		code = 1
		msg = "删除成功"
...
func (c *OrganizationAddController) Post() {
	var code int
	var msg string
	var url = web.URLFor("OrganizationIndexController.Get")
	defer sysmanage.Retjson(c.Ctx, &msg, &code, &url)
	model := Organization{}
...
func (c *OrganizationEditController) Get() {
	...
	if err == orm.ErrNoRows || err == orm.ErrMissPK {
		c.Redirect(web.URLFor("OrganizationIndexController.get"), 302)
		return
	}
	c.Data["data"] = &model
