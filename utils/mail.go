...
func (this *MailSender) Send() error {
	...
	if err := d.DialAndSend(m); err != nil {
		logs.Error("MailSender send error", err.Error())
		return err
	}
	logs.Info("Mail send success")
	return nil
}
...
func (this *MailSender) SendSeparate() (int, error) {
	...
	s, err := d.Dial()
	if err != nil {
		logs.Error("MailSender SendSeparate Dial error", err.Error())
		return 0, err
	}
	...
	for _, v := range this.To {
		...
		if err := gomail.Send(s, m); err != nil {
			logs.Error("MailSender SendSeparate error", err.Error())
		} else {
			num++
		}
		m.Reset()
...
func SendMailVerifyCode(to string) error {
	...
	if err != nil {
		...
		logs.Info("Send mail verify code to:", to, "verify code:", vc)
		return nil
	}
}
